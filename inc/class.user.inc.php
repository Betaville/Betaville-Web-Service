...
class UserActions{
	...
	public function addUser($username, $password, $emailAddress, $code){
		...
		if(!($this->isUsernameAvailable($username))) return -4;
			...
			$activated = 0;
			$sql = "INSERT INTO user (userName, strongpass, strongsalt, email, activacted, code) VALUES (:username, :strongpass, :strongsalt, :email, :activacted, :code)";
			...
			try{
				...
				$stmt->bindParam(":activacted", $activated, PDO::PARAM_STR);
	...
	public function isUserActivated($username){
	 ...
	 public function validateUser($sCode){
		$codeSQL = "UPDATE user SET activacted=1 where code=:secretCode";
		try{
